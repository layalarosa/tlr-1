class Character {
    ...
    getAtk() {
        ...
        if (this.equipment.weapon) {
            var item = getItemStats(this.equipment.weapon);
    ...
    getDef() {
        ...
        if (this.equipment.body) {
            var bodyItem = getItemStats(this.equipment.body);
            if (bodyItem) def += bodyItem.def || 0;
        }
        if (this.equipment.offhand) {
            var offItem = getItemStats(this.equipment.offhand);
            if (offItem) def += offItem.def || 0;
        }
        return def;
    }
    ...
    levelUp() {
        ...
        this.stats.luk += Math.floor(Math.random() * 2) + 1;
    ...
    takeDamage(amount, isMagic) {
        var def = isMagic ? this.getMagDef() : this.getDef();
        if (this.defending) def = Math.floor(def * 1.5);
        const actualDmg = Math.max(1, amount - def);
        this.hp -= actualDmg;
        if (this.hp <= 0) {
            this.hp = 0;
            this.alive = false;
    ...
    getMagDef() {
        return Math.floor(this.stats.wis / 2) + Math.floor(this.level / 3);
    }
    ...
    _getSpellCost(spellId) {
        const costs = {
            ...
            entangle: 5,
            powerStrike: 4, warCry: 3, backstab: 5,
            assassinate: 8, poisonBlade: 4, archery: 3,
            disarmTrap: 4, pickLock: 3, healAlly: 6,
            vanish: 5, track: 2
        };
    ...
    static fromSaveData(data) {
        ...
        char.row = data.row;
        char.defending = data.defending || false;
        char.blessed = data.blessed || false;
        char.statusEffects = data.statusEffects || [];
